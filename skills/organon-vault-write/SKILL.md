...
- **`createTargetIfMissing: false`** explicite sur tout `patch_vault_file targetType: heading`. Native default depuis istefox 0.4.0 (#76) — passer le flag explicite reste defense-in-depth contre toute future régression et documente l'intent du caller dans la trace MCP.
- **First heading patch of session** : post-write verify mtime + diff. Confiance après. Pourquoi : la confiance se construit par session, pas globalement. Garde-fous istefox 0.4.x : H2 racine sans parent H1 = fail-loud reject (0.4.2 #80) ; block ref dans une cellule de table = fail-loud reject (0.4.2 #81) ; block ref sur frontière de fenced-code = fail-loud reject (0.4.3 #84). Pas de corruption silencieuse mais pas d'écriture non plus — le caller doit choisir une autre voie (cf. pitfall tables + block refs dans `organon-markdown-style`).
