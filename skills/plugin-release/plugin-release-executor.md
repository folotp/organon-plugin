...
# plugin-release-executor

End-to-end runbook for cutting a `folotp/organon-plugin` release. You are the post-pre-flight executor: by the time you run, `release-readiness` has cleared the gates. The `.plugin` archive is gitignored (`.gitignore` line `*.plugin`) — you build it and upload it as a Release asset, never commit it. The marketplace (`folotp/claude-marketplace`) consumes the asset URL.
...
gh release create v<version> \
    ...
    --notes "<body — see skills/plugin-release/references/release-notes-template.md>"
```
...
- `skills/plugin-release/references/release-notes-template.md` — body template for `gh release create --notes`.
